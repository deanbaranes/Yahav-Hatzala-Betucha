                          ...
                          <div className="pt-2 mt-2 border-t border-gray-200">
                            ...
                            <ul className="list-none text-gray-600 text-xs space-y-1">
                              {client.trips_details.map((trip: any, i: number) => (
                                <li key={i} className="flex flex-wrap items-center gap-1.5 bg-gray-50 px-2 py-1.5 rounded border border-gray-100">
                                  <span className="font-semibold text-gray-700">
                                    תאריך {trip.date}{trip.location ? ` - ${trip.location}` : ''}:
                                  </span>
                                  <span className="flex items-center gap-1.5">
                                    <span className={`font-bold ${trip.planned_hours >= 8 ? 'text-red-600' : 'text-gray-600'}`}>
                                      {trip.planned_hours} שעות מתוכננות
                                    </span>
                                    {trip.nights > 0 && (
