...
def update_employee_details(
    ...
    if data.phone is not None:
        ...
        existing = (
            db.query(User).filter(User.phone == data.phone, User.id != user_id).first()
        )
        if existing:
            raise HTTPException(
                status_code=400, detail="מספר טלפון זה כבר קיים עבור משתמש אחר"
            )
...
def get_employees(
    ...
    query = db.query(User).filter(
        User.role.in_([UserRole.employee, UserRole.admin]), User.status != "inactive"
    )
    ...
    if month and year:
        ...
        assigned_users = (
            db.query(TripAssignment.user_id)
            .join(Trip)
            .filter(
        ...
        adjusted_users = db.query(PayrollAdjustment.user_id).filter(
            PayrollAdjustment.month == month, PayrollAdjustment.year == year
        )
    ...
    return [
        {
            ...
            if hasattr(e.employment_type, "value")
            else (e.employment_type or EmploymentType.EMPLOYEE.value),
        }
        for e in employees
    ]
...
def get_pending_employees(
    ...
    pending = (
        db.query(User)
        .filter(User.role == UserRole.employee, User.status == "pending")
        .order_by(User.full_name.asc())
        .all()
    )
    return [
        {"id": str(e.id), "full_name": e.full_name, "phone": e.phone} for e in pending
    ]
...
def reject_employee(
    ...
    db.query(TripAssignment).filter(TripAssignment.user_id == user_id).delete(
        synchronize_session=False
    )
    db.query(PayrollAdjustment).filter(PayrollAdjustment.user_id == user_id).delete(
        synchronize_session=False
    )
    db.query(Payslip).filter(Payslip.user_id == user_id).delete(
        synchronize_session=False
    )
...
def get_adjustments(
    ...
    return [
        {"id": str(a.id), "type": a.type, "amount": float(a.amount), "notes": a.notes}
        for a in adjs
    ]
...
def export_all_payroll(
    ...
    adjusted_users = (
        ...
        .filter(PayrollAdjustment.month == month, PayrollAdjustment.year == year)
        .subquery()
    )
...
async def upload_payslip(
    ...
    try:
        ...
        payslip = Payslip(user_id=user.id, month=month, year=year, file_url=file_url)
        db.add(payslip)
        db.commit()
        db.refresh(payslip)
...
def get_user_payslips(
    ...
    payslips = (
        db.query(Payslip)
        .filter(Payslip.user_id == user_id)
        .order_by(Payslip.year.desc(), Payslip.month.desc())
        .all()
    )
    return [
        {"id": str(p.id), "month": p.month, "year": p.year, "file_url": p.file_url}
        for p in payslips
    ]
