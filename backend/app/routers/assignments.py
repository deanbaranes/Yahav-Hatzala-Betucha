...
from app.schemas import AdminAssignRequest, JoinTripRequest
...
def get_unconfirmed_assignments(
...
def confirm_assignment(
    ...
    assignment = (
        db.query(TripAssignment).filter(TripAssignment.id == assignment_id).first()
    )
    if not assignment:
    ...
    if user:
        ...
        contact_str = (
            f"פרטי איש קשר לטיול: {' - '.join(contact_parts)}" if contact_parts else ""
        )
...
def delete_assignment(
    ...
    next_in_line = (
        db.query(TripAssignment)
        .filter(
            TripAssignment.trip_id == trip_id, TripAssignment.status == "waitlisted"
        )
        .with_for_update()
        .order_by(TripAssignment.created_at.asc())
        .first()
    )
...
def join_trip(
    ...
    if max_capacity == 0 and not trip.roles_requirements:
        ...
        assigned_count_for_role = (
            db.query(TripAssignment)
            .filter(
                TripAssignment.trip_id == trip.id, TripAssignment.status == "assigned"
            )
            .count()
        )
    ...
    total_assigned = (
        db.query(TripAssignment)
        .filter(TripAssignment.trip_id == trip.id, TripAssignment.status == "assigned")
        .count()
    )

    status = (
        "waitlisted"
        if (assigned_count_for_role >= max_capacity or total_assigned >= trip.capacity)
        else "assigned"
    )

    new_assignment = TripAssignment(
        trip_id=trip.id, user_id=current_user.id, status=status, role=request.role
    )
...
def cancel_trip(
        ...
        if next_in_line:
            ...
            if next_in_line.user:
                ...
                NotificationService.create_in_app_notification(
                    promoted_msg, db, user_id=next_in_line.user_id
                )
    ...
    return {
        ...
        "promoted_user": str(promoted_user) if promoted_user else None,
    }


@router.post("/{trip_id}/assign")
def admin_assign_trip(
    trip_id: str,
    ...
    existing = (
        db.query(TripAssignment)
        .filter(
            TripAssignment.trip_id == trip_id, TripAssignment.user_id == request.user_id
        )
        .first()
    )
...
def admin_remove_trip_assignment(
    trip_id: str,
    user_id: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_admin_user),
    ...
    assignment = (
        db.query(TripAssignment)
        .filter(TripAssignment.trip_id == trip_id, TripAssignment.user_id == user_id)
        .first()
    )
