...
def create_supplier(
    ...
    contact = (
        db.query(SupplierContact).filter(SupplierContact.name == supplier.name).first()
    )
    if not contact:
...
def upload_supplier_receipt(
    ...
    for i, file in enumerate(files):
        try:
            ...
            raise HTTPException(
                status_code=500, detail="שגיאה בהעלאת הקובץ. אנא נסה שנית."
            )
        ...
        safe_filename = (
            ...
            if file.filename
            else f"supplier_receipt_{supplier.name}_{now.date()}_{i}.jpg"
        )
