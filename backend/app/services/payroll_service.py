...
class PayrollService:
    ...
    def generate_employee_report(self, user: User, month: int, year: int) -> dict:
        if user.status == "inactive":
            raise ValueError(
                f"Cannot generate payroll for inactive user {user.full_name}"
            )
        ...
        for r in reports:
            ...
            if hasattr(r, "sleeps") and r.sleeps is not None:
                ...
                if (
                    trip_has_accom
                    and r.assignment.trip.start_date
                    and r.assignment.trip.end_date
                ):
                    trip_nights = (
                        r.assignment.trip.end_date.date()
                        - r.assignment.trip.start_date.date()
                    ).days
                    if trip_nights > 0:
            ...
            if r.assignment.trip.global_salary:
                ...
                if r.daily_shifts and len(r.daily_shifts) > 0:
                    for shift in r.daily_shifts:
                        if "start_time" in shift:
                            report_days_set.add(
                                datetime.fromisoformat(shift["start_time"]).date()
                            )
                elif r.start_time:
        ...
        if is_global_salary:
            ...
            total_hours += (
                global_money / Decimal("60") if global_money > 0 else Decimal(0)
            )
    ...
    def _format_report(
        ...
        report_text += (
            f"נסיעות: {travel_pay.quantize(Decimal('0.00'), rounding=ROUND_HALF_UP)} ₪"
        )
    ...
    def create_supplier_record_from_report(self, report: TripReport):
        if (
            ...
            and report.assignment.user
        ):
            user = report.assignment.user
            if user.employment_type == EmploymentType.FREELANCER:
                from app.models.supplier import Supplier

                existing = (
                    self.db.query(Supplier)
                    .filter(Supplier.report_id == report.id)
                    .first()
                )
                if not existing:
                    ...
                    trip_loc = (
                        report.assignment.trip.location
                        if report.assignment.trip
                        else ""
                    )
                    role = (
                        report.assignment.role
                        if report.assignment.role
                        else "תפקיד כללי"
                    )
                    ...
                    if rounded_ot > 0:
                        ot_display = (
                            int(rounded_ot) if rounded_ot.is_integer() else rounded_ot
                        )
                        details_text += f" | {ot_display} שעות נוספות"

                    supplier_entry = Supplier(
                        name=user.full_name,
                        debt_date=report.start_time.date()
