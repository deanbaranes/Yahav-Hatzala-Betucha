...
from app.models.trip_report import BillingStatus, ManagerStatus, TripReport
...
def check_client_debts():
    ...
    clients = (
        db.query(Client)
        .filter(Client.debt_start_date.isnot(None), Client.numeric_balance < 0)
        .all()
    )
    ...
    for client in clients:
        ...
        if client.payment_terms and re.search(r"\d+", client.payment_terms):
            ...
            if debt_start.month == 12:
                next_month = debt_start.replace(
                    year=debt_start.year + 1, month=1, day=1
                )
            else:
        ...
        if now > due_date:
            ...
            recent_notif = (
                ...
                .filter(
                    Notification.message == msg, Notification.created_at >= last_week
                )
                .first()
            )
...
def check_unassigned_trips():
    ...
    trips = (
        db.query(Trip)
        .filter(Trip.start_date > now, Trip.start_date <= in_48_hours)
        .all()
    )
    ...
    for trip in trips:
        confirmed_assignments = [
            ...
            if a.is_confirmed and a.status == AssignmentStatus.assigned
        ]
...
def delete_old_business_expenses():
    ...
    old_expenses = (
        ...
        .filter(
            ...
            | (
                (BusinessExpense.expense_year == cutoff_year)
                & (BusinessExpense.expense_month <= cutoff_month)
            )
        )
        .all()
    )
...
def check_ended_trips_for_reports():
    ...
    try:
        ...
        recent_trips = (
            ...
            .filter(Trip.start_date >= three_days_ago)
            .all()
        )
                ...
                for assignment in trip.assignments:
                    if (
                        ...
                        new_report = TripReport(
                            ...
                            sleeps=0,
                            is_draft=False,
                            manager_status=ManagerStatus.approved,
...
def check_upcoming_trips_for_confirmation():
    ...
    try:
        ...
        upcoming_trips = (
            db.query(Trip)
            .filter(Trip.start_date >= tomorrow_start, Trip.start_date <= tomorrow_end)
            .all()
        )
                ...
                if (
                    ...
                    if user and user.phone:
                        ...
                        if not existing_notif:
                            ...
                            NotificationService.create_in_app_notification(
                                message=msg, db=db, user_id=assignment.user_id
                            )
    finally:
        db.close()
        ...
        for trip in upcoming_trips:
            for assignment in trip.assignments:
                if (
                    assignment.is_confirmed
                    and assignment.status == AssignmentStatus.assigned
                    and not assignment.employee_confirmed_arrival
                ):
                    user = assignment.user
                    if user:
                        ...
                        existing_notif = (
                            db.query(Notification)
                            .filter(Notification.message == msg)
                            .first()
                        )
...
def check_unpaid_suppliers():
    ...
    try:
        unpaid_suppliers = (
            db.query(Supplier).filter(Supplier.is_invoiced == False).all()
        )
        ...
        if admin_phone:
            NotificationService.send_sms(admin_phone, msg, db=db)
            NotificationService.create_in_app_notification(
                msg, db, title="תזכורת תשלום לספקים"
            )
    except Exception as e:
